...
function HelpPage() {
  ...
  const filteredArticles = search.trim()
    ? ARTICLES.filter(a => a.t.toLowerCase().includes(search.toLowerCase()) || a.c.toLowerCase().includes(search.toLowerCase()))
    : ARTICLES;
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="space-y-3">
          ...
          {filteredArticles.length === 0 && (
            <p className="text-sm text-muted-foreground px-1">Aucun article trouvé.</p>
          )}
          {filteredArticles.map(a => (
            <div key={a.t} className="rounded-xl border border-border bg-card/40 p-4">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-border bg-card/40 p-5">
            ...
            <a href="mailto:contact@convertilab.com">
              <Button size="sm" className="w-full">Envoyer un message</Button>
            </a>
          </div>
        </div>
      </div>
