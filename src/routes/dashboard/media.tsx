...
function MediaPage() {
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    for (const file of Array.from(files)) {
      ...
      if (file.size > MAX_SIZE) { toast.error(`${file.name} dépasse 5 Mo`); continue; }
  ...
  return (
    <div className="p-6 md:p-8 space-y-6">
      ...
      >
        ...
        <p className="text-xs text-muted-foreground mt-1">Logo, photos d'annonces, portrait… JPG/PNG/WEBP, 5 Mo max par fichier</p>
        <Button className="mt-4" onClick={() => inputRef.current?.click()} disabled={uploading}>
          {uploading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Upload en cours…</> : <><Upload className="h-4 w-4 mr-2" /> Choisir des fichiers</>}
        </Button>
      </div>
              ...
              <div key={p.path} className="group relative rounded-xl overflow-hidden border border-border bg-muted/20 aspect-square">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/0 to-black/0 opacity-0 group-hover:opacity-100 transition flex flex-col justify-end p-2 gap-1">
                  <div className="flex gap-1">
                    <Button size="sm" variant="secondary" className="h-7 flex-1 text-[11px]" onClick={() => copy(p.url)}>
                      {copied === p.url ? <Check className="h-3 w-3" /> : <Copy className="h-3 w-3" />}
                    </Button>
                    <Button size="sm" variant="destructive" className="h-7 px-2" onClick={() => remove(p.path)}>
