  ...
  reactExports.useEffect(() => {
    ...
    async function load() {
      ...
      if (profileRes.data?.id) {
        ...
        const {
          data: events
        } = await supabase.from("nfc_analytics").select("event_type, created_at, event_data").eq("profile_id", profileId).order("created_at", {
          ascending: true
        });
        setAnalytics(events ?? []);
        channel = supabase.channel(`analytics-${profileId}`).on("postgres_changes", {
          event: "INSERT",
          schema: "public",
          table: "nfc_analytics",
