...
import { g as getProfileMeta } from "./router-DZxGzuQJ.mjs";
...
function StatistiquesPage() {
  ...
  const doFetch = reactExports.useCallback((pid) => {
    supabase.from("nfc_analytics").select("event_type, created_at, event_data").eq("profile_id", pid).order("created_at", {
      ascending: true
    }).then(({
      data
    }) => {
