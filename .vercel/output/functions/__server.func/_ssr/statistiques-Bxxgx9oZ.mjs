...
import { g as getProfileMeta } from "./router-DIf0f6dh.mjs";
...
function StatistiquesPage() {
  ...
  const fetchAnalytics = reactExports.useCallback(async (pid) => {
    const {
      data: events
    } = await supabase.from("nfc_analytics").select("event_type, created_at, event_data").eq("profile_id", pid).order("created_at", {
      ascending: true
    });
    setAnalytics(events ?? []);
  }, []);
