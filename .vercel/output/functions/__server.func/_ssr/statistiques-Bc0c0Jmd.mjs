...
import { g as getProfileMeta } from "./router-Id1jivg0.mjs";
...
function StatistiquesPage() {
  ...
  const [dbg, setDbg] = reactExports.useState("init");
  const loadData = reactExports.useCallback(async (pid) => {
    setDbg(`calling rpc with ${pid.slice(0, 8)}`);
    const {
      data,
      error
    } = await supabase.rpc("get_card_analytics", {
      p_profile_id: pid
    });
    if (error) {
      setDbg(`ERROR: ${error.message}`);
    } else {
      setDbg(`OK: ${data?.length ?? 0} rows, clicks=${(data ?? []).filter((r) => r.event_type === "click_button" || r.event_type === "click_social").length}`);
    }
    setAnalytics(data ?? []);
    setLoading(false);
    setRefreshing(false);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "p-5 lg:p-8 max-w-5xl space-y-6", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-xs font-mono bg-yellow-100 text-yellow-900 rounded px-3 py-2 border border-yellow-300", children: [
      "🔍 DEBUG: pid=",
      profileId?.slice(0, 8) ?? "null",
      " | ",
      dbg
    ] }),
