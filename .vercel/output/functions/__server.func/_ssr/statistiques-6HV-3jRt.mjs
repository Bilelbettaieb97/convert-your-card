...
import { g as getProfileMeta } from "./router-Bvednol5.mjs";
import "../_libs/sonner.mjs";
import "../_libs/seroval.mjs";
import { as as Lock, m as Eye, n as MousePointerClick, r as UserPlus, p as TrendingUp, ai as ChartColumn, k as MessageCircle, l as Check, ak as Copy, Q as QrCode, Z as Zap, aO as ArrowUpRight, aP as Target, a5 as Share2, D as Download, a2 as Calendar, G as Globe, M as Mail, au as Phone } from "../_libs/lucide-react.mjs";
...
function StatistiquesPage() {
  ...
  reactExports.useEffect(() => {
    ...
    async function load() {
      ...
      const {
        data: events
      } = await supabase.from("nfc_analytics").select("event_type, created_at, event_data").eq("profile_id", profile.id).order("created_at", {
        ascending: true
      });
      setAnalytics(events ?? []);
      channel = supabase.channel(`analytics-${profile.id}`).on("postgres_changes", {
        event: "INSERT",
        schema: "public",
        table: "nfc_analytics",
